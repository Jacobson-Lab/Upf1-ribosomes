...
B <- ggplot(toplot_filter[toplot_filter$sample %in% xx, ], aes(x = IPvsTotal_change, y = RO_log2FoldChange)) +
  ...
  stat_pvalue_manual(data = stat.test, label = "p.adj.signif", tip.length = 0.005, size = 6/.pt, vjust = 0.3) +
  geom_text(data = nn[nn$sample %in% xx, ], aes(x = IPvsTotal_change, y = -8, label = paste0("n = ", Freq)), size = 6/.pt) +
...
C <- ggplot(toplot[toplot$sample %in% xx, ], aes(x = IPvsTotal_change, y = l_cds)) +
  geom_boxplot(aes(fill = IPvsTotal_change), outlier.color = NA, width = 0.75, size = 0.5) +
  stat_pvalue_manual(data = stat.test, label = "p.adj.signif", tip.length = 0.005, size = 6/.pt, vjust = 0.3) +
  geom_text(data = nn[nn$sample %in% xx, ], aes(x = IPvsTotal_change, y = -0.5, label = paste0("n = ", Freq)), size = 6/.pt) +
  facet_nested(.~sample+NMD, nest_line = TRUE) +
  scale_fill_manual(name = "", values = c("grey70", "orange", "purple")) +
...
D <- ggplot(toplot[toplot$sample %in% xx, ], aes(x = IPvsTotal_change, y = tAI_cds)) +
  geom_boxplot(aes(fill = IPvsTotal_change), outlier.color = NA, width = 0.75, size = 0.5) +
  stat_pvalue_manual(data = stat.test, label = "p.adj.signif", tip.length = 0.005, size = 6/.pt, vjust = 0.3) +
  geom_text(data = nn[nn$sample %in% xx, ], aes(x = IPvsTotal_change, y = 0.2, label = paste0("n = ", Freq)), size = 6/.pt) +
  facet_nested(.~sample+NMD, nest_line = TRUE) +
  scale_fill_manual(name = "", values = c("grey70", "orange", "purple")) +
...
cairo_pdf(filename = "IPvsTotal_characteristics_7.5x11.pdf", family = "Arial", width = 7.5, height = 11) 
p
dev.off()
```
