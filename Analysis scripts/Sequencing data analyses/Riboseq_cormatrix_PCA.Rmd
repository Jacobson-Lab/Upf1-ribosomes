...
cormat_ribo <- ggplot(R_cor) +
  ...
  theme(panel.grid = element_blank(), panel.spacing = unit(0.1, "cm"), panel.border = element_rect(size = 0.2),
        ...
        legend.position = "top", legend.key.height = unit(0.25, "cm"), legend.key.width = unit(1, "cm"), aspect.ratio = 1)
```
...
pca_ribo <- ggplot(pcaData, aes(x = PC1, y = PC2, color = strain)) +
  ...
  geom_point(aes(shape = riboCHX), alpha = 1, size = 2, stroke = 0.5) +
  ...
  #guides(color = guide_legend(title = "Strain", label.theme = element_text(face = "italic", size = 8), byrow = TRUE, 
   #                           order = 1, nrow = 2),
    #     shape = guide_legend(order = 2, nrow = 1))
  theme(legend.position = "right", legend.box = "vertical", legend.key.height = unit(0.3, "cm"), aspect.ratio = 2.75/7) +
...
#### Combine plots
```{r}
p <- ggarrange(cormat_ribo, pca_ribo, nrow = 2, labels = "AUTO", heights = c(0.75, 0.25), font.label = list(size = 12, face = "bold"))
```
```{r}
library(Cairo)
CairoFonts(
...
cairo_pdf(filename = "Replicate_reproducibility_Riboseq_8x11_v4.pdf", family = "Arial", width = 8, height = 11) 
p
dev.off()
```
