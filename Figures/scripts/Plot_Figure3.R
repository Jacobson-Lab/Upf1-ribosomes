...
mainp <- ggplot(df) +
  ...
  geom_text(data = size_labels, aes(x = xpos, y = ypos, label = labs), size = 2, fontface = "bold") +
...
sp <- ggplot(size_fig) +
  ...
  geom_text(aes(x = left_pos, y = y, label = left_labs), size = 2, fontface = "bold", hjust = 1) +
  geom_text(aes(x = right_pos, y = y, label = right_labs), size = 2, fontface = "bold", hjust = 0) +
...
cairo_pdf(filename = "Footprint_ends_mapping_5x5.pdf", family = "Arial", width = 5, height = 5) 
p
dev.off()
