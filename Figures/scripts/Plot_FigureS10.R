...
# Prepare Data
  # N-terminal data
dfa <- read.table("../data/Data_FigureS10a_Nterm.txt", header = TRUE, sep = "\t")
...
dfa$riborep <- sub("_.*_", " rep ", dfa$sample)
  ...
  # C-terminal data
dfb <- read.table("../data/Data_FigureS10b_Cterm.txt", header = TRUE, sep = "\t")
...
# Prepare Data
  # N-terminal data
dfc <- read.table("../data/Data_FigureS10c_Nterm.txt", header = TRUE, sep = "\t")
...
library(patchwork)
S10 <- pa + pb + pc + pd +
  plot_layout(heights = c(16, 21), widths = c(2.5, 5.5), byrow = FALSE) +
...
cairo_pdf(filename = "psite_region_reading_frame_8x10.pdf", family = "Arial", width = 8, height = 10) 
S10
dev.off()
